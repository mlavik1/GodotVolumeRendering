...
{
    public event Action<VolumeDataset> onDatasetLoaded;
    ...
    public RawDatasetImporterDialog(string datasetPath, int dimX, int dimY, int dimZ, DataContentFormat dataFormat, Endianness endianness, int bytesToSkip)
    {
        ...
        this.Confirmed += () => {
            ...
            onDatasetLoaded?.Invoke(dataset);
        };
    }

    public override void _EnterTree()
    {
        contentControl = GD.Load<PackedScene>("res://addons/godot_volume_rendering/gui/RawDatasetImporterDialog.tscn").Instantiate<RawDatasetImporterControl>();
